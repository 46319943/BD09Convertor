...
def convertMC2LL(lng, lat):
    """Convert BD09 Mercator (BD09MC) to BD09 longitude/latitude (BD09LL)."""
    lng_abs = abs(lng)
    lat_abs = abs(lat)

    cF = MC2LL[-1]
    for cE in range(len(MCBAND)):
        if lat_abs >= MCBAND[cE]:
            cF = MC2LL[cE]
            break
...
def convertLL2MC(lng, lat):
    """Convert BD09 longitude/latitude (BD09LL) to BD09 Mercator (BD09MC)."""
    cE = None
...
def convertBD09LL2WGS84(lng, lat):
    """Convert BD09LL to WGS84."""
    gcj_lng, gcj_lat = bd09_to_gcj02(lng, lat)
    return gcj02_to_wgs84(gcj_lng, gcj_lat)


def convertWGS842BD09LL(lng, lat):
    """Convert WGS84 to BD09LL."""
    gcj_lng, gcj_lat = wgs84_to_gcj02(lng, lat)
    return gcj02_to_bd09(gcj_lng, gcj_lat)


def convertBD09MC2WGS84(lng, lat):
    """Convert BD09MC to WGS84."""
    bd_lng, bd_lat = convertMC2LL(lng, lat)
    return convertBD09LL2WGS84(bd_lng, bd_lat)


def convertWGS842BD09MC(lng, lat):
    """Convert WGS84 to BD09MC."""
    bd_lng, bd_lat = convertWGS842BD09LL(lng, lat)
    return convertLL2MC(bd_lng, bd_lat)


def convertor(lng, lat, cE):
    T = cE[0] + cE[1] * abs(lng)
    cC = abs(lat) / cE[9]
    cF = (
        cE[2] + cE[3] * cC + cE[4] * cC * cC + cE[5] * cC * cC * cC + cE[6] * cC * cC * cC * cC
        + cE[7] * cC * cC * cC * cC * cC + cE[8] * cC * cC * cC * cC * cC * cC
    )
    T = T * (-1 if lng < 0 else 1)
    cF = cF * (-1 if lat < 0 else 1)

    return T, cF


def getRange(cD, cC, T):
    cD = min(cD, T)
    cD = max(cD, cC)
    return cD
...
if __name__ == '__main__':
    ...
    print(convertBD09LL2WGS84(116.28112917752422, 39.96844549691906))
    print(convertWGS842BD09LL(116.274625, 39.961627))
